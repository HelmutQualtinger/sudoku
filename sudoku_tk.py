...
class SudokuTk(tk.Tk):
    ...
    def set_field(self, event, row, col, bt, action):
        ...
        if action == "Release" and bt == 1 and not self.board.frozen2d[row, col]:
            ...
            return
        if action == "Press" and bt == 1 and not self.board.frozen2d[row, col]:
    ...
    def update_widgets(self):
        self.free_fields.config(text="Free fields: " +
                                str(self.board.count_empty_fields()))
        for (row, col) in itertools.product(range(9), range(9)):  # over all rows and columns
            ...
            if self.board.board2d[row, col] == 0:
                ...
                self.Buttons[(row, col)].config(
                    bg="#80ff80", fg="#80ff80", relief=tk.RAISED)
            # frozen fields are gray
            if self.board.frozen2d[row, col]:
                self.Buttons[(row, col)].config(text=str(
                    self.board.board2d[row, col]), bg="#c0c0c0", fg="#000000", relief=tk.SUNKEN)
    ...
    def load_file(self, filename=None, ignore_discard=False, ignore_expires=False):
        ...
        filename = filedialog.askopenfilename(
            title="Select Sudokufile:", filetypes=[("JSON Files", "*.json")])
        if filename:
